...
-- Doanh thu của cửa hàng theo tháng
CREATE VIEW Doanh_thu_cua_hang_theo_thang AS
WITH CTE_2022 AS (
SELECT DATEPART(YEAR,[Thời gian tạo đơn]) AS Year, DATEPART(MONTH,[Thời gian tạo đơn]) AS Month,SUM([Thành tiền]) AS Revenue,SUM(SL) AS Quantity FROM Sales
WHERE DATEPART(YEAR,[Thời gian tạo đơn]) = 2022
...
-- Doanh thu và số lượng của đơn hàng theo từng năm
SELECT DATEPART(YEAR,[Thời gian tạo đơn]) AS Year, SUM([Thành tiền]) AS Revenue, COUNT(DISTINCT([Mã Đơn Hàng])) AS Quantity FROM Sales
GROUP BY DATEPART(YEAR,[Thời gian tạo đơn])
ORDER BY YEAR ASC
...
SELECT 
    ...
    SUM([Thành tiền]) / COUNT(DISTINCT [Mã đơn hàng]) AS [Doanh Thu TB (AOV)]
FROM Sales
GROUP BY 
    CASE 
        WHEN DATEPART(HOUR, [Thời gian tạo đơn]) >= 8 AND DATEPART(HOUR, [Thời gian tạo đơn]) < 13 THEN N'Ca Sáng (8h-13h)'
        ...
        ELSE N'Ngoài giờ hành chính'
    END
ORDER BY [Doanh Thu TB (AOV)] DESC;
